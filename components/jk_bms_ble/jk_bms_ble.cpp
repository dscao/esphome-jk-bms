...
static const uint8_t FRAME_VERSION_JK_PB = 0x04; 
...
static constexpr const char *const ERRORS[ERRORS_SIZE] = {
    ...
    "Mosfet overtemperature", "Cell count is not equal to settings", "Current sensor anomaly",
    "Cell Overvoltage", "Battery pack overvoltage", "Charge overcurrent protection", "Charge short circuit"
};

uint8_t crc(const uint8_t data[], const uint16_t len) {
  uint8_t crc = 0;
  for (uint16_t i = 0; i < len; i++) crc = crc + data[i];
  return crc;
}
...
bool JkBmsBle::write_register(uint8_t address, uint32_t value, uint8_t length) {
  ...
  for (int i = 10; i < 19; i++) frame[i] = 0x00;
...
void JkBmsBle::decode_(const std::vector<uint8_t> &data) {
  ...
  if (frame_type == 0x02) {
    if (this->protocol_version_ == PROTOCOL_VERSION_JK_PB) {
      this->decode_jk_pb_cell_info_(data);
    } else if (this->protocol_version_ == PROTOCOL_VERSION_JK04) {
      this->decode_jk04_cell_info_(data);
    } else {
      this->decode_jk02_cell_info_(data);
    }
  } else if (frame_type == 0x01) {
    (this->protocol_version_ == PROTOCOL_VERSION_JK04) ? this->decode_jk04_settings_(data) : this->decode_jk02_settings_(data);
...
void JkBmsBle::decode_jk_pb_cell_info_(const std::vector<uint8_t> &data) {
  ...
  ESP_LOGD(TAG, "Parsing JK_PB frame...");

  // 1. 电芯电压 (从 6 位起，共 32 串位)
  float sum_v = 0, min_v = 10.0f, max_v = 0;
  uint8_t count = 0;
  for (uint8_t i = 0; i < 32; i++) {
    float v = (float)jk_get_16bit(6 + i * 2) * 0.001f;
    if (v > 0.5f && v < 5.0f) {
  ...
  if (count > 0) {
    this->publish_state_(this->average_cell_voltage_sensor_, sum_v / count);
  ...
  this->publish_state_(this->total_voltage_sensor_, (float)jk_get_32bit(150) * 0.001f);
  
  float pwr = (float)((int32_t)jk_get_32bit(154)) * 0.001f;
  this->publish_state_(this->power_sensor_, pwr);
  this->publish_state_(this->current_sensor_, (float)((int32_t)jk_get_32bit(158)) * 0.001f);
  
  this->publish_state_(this->temperatures_[0].temperature_sensor_, (float)((int16_t)jk_get_16bit(162)) * 0.1f);
  this->publish_state_(this->temperatures_[1].temperature_sensor_, (float)((int16_t)jk_get_16bit(164)) * 0.1f);
